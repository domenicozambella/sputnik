...
For any $\epsilon>0$ and $d\in\omega$ there is some $N=N(\epsilon,k)$ such that for every set system $\Phi$ of \vc-dimension $\le k$, there is an $\epsilon$-net of cardinality $\le N$. By Remark~\ref{rmk_nets} we obtain an $1/r$-net of cardinality $\le c\,r\,\ln r$.
\end{proposition}
...
\begin{proof}
Let $r=\tau^*(\Phi)$ ad let $A$ be an optimal fractional transversal. As $\Phi$ is finite we can assume that the support of $A$ is finite. After normalizing, $A$ defines a probability measure on $\U$ concentrated on $\supp A$. Namely, $\Pr(\{x\})=(1/r)A(x)$ for all $x\in\U$. By the definition of fractional transversal every set in $\Phi$ has measure at last $1/r$.  
\end{proof}
