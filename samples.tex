...
\ceq{}{\ge}{\frac{|C\cap\B|}{n^+}\ -\ \delta_{A,C}}
...
Fix an enumeration $a=\<a_1,\dots,a_n\>$ of $A$. A tuple $c=\<c_1,\dots,c_n\>\in\{\pm1\}^n$ is called a \emph{coloring}. To each coloring $c$ we associate the subsample $C_{a,c}\subseteq A$ which assigns to $x\in\U$ multiplicity $\big|\{i : a_i=x,\ c_i=+1\}\big|$.

Note that the quantity 

\ceq{\hfill\Delta_{a,c,\B}}{=}{\sum_{a_i\in\B}c_i.}

coincides with $\Delta_{A,C_{a,c},\B}$.

There is probability measure on the subsamples of $A$ which is computationally convenient. We assume that $C=C_{a,c}$ where $c=\<c_1,\dots,c_n\>\in\{\pm1\}^n$ is obtained tossing $n$ times a fair coin.

That is, we assume an uniform distribution on the colorings and this induces a multivariate binomial distribution on the subsamples. Note that this distribution is independent of the enumeration of $A$.
...
\ceq{\hfill\Pr \Big(\big|X-{\rm E}(X)\big|\ge\epsilon\Big)}{\le}{\frac{{\rm Var}(X)}{\epsilon^{\scriptscriptstyle 2}}}
\end{void_thm}
...
\ceq{}{=}{\epsilon^2 \Pr \Big(\big|X-{\rm E}(X)\big|\le\epsilon\Big)}
\end{proof}

\begin{void_thm}[Proposition (Weak Law of Large Numbers)]\label{prop_wlln}
...
\ceq{\hfill\Pr \Big(\big|p -\bar X \big|\ge\epsilon\Big)}{\le}{\frac{1}{4n\epsilon^{\scriptscriptstyle 2}}}\hfill where\quad$\displaystyle\bar X=\frac1n\sum^n_{i=1}X_i$
...
\begin{lemma}[(Chernoff's bound)]\label{Chernoff}
For $i=1,\dots,n$ let $X_i$ be independent identically distributed random variables such that\/ $\Pr(X_i{=}1)=\Pr(X_i{=}{-}1)=1/2$.
Then for every $\epsilon>0$

\ceq{\hfill \Pr\big(M\ge\epsilon\big)}{\le}{\exp\Big(-\frac{\epsilon^2}{2n}\Big)}\hfill where $\displaystyle M=\sum^n_{i=1}X_i$
\end{lemma}
\begin{proof}
...
\ceq{\flat\hfill {\rm E}\big(e^{tX_i}\big)}{=}{\frac12e^t\ +\ \frac12e^{-t}}
...
\begin{void_thm}[Proposition (Hoeffding's inequality )]\label{Chebyshev}
...
\ceq{\hfill\Pr \big(M\ge\epsilon\big)}{\le}{\exp{\Big(-\frac{2\epsilon^2}{d}}\Big)}.
\end{void_thm}
...
\ceq{\hfill e^{tX_i}}{=}{e^{td_iZ_i-td_i(1-Z_i)}}

\ceq{}{\le}{Z_i\, e^{td_i}+(1-Z_i)\,e^{-td_i}}

As ${\rm E}[Z_i]=1/2$ we obtain

\ceq{\hfill {\rm E}\big(e^{tX_i}\big)}{\le}{\frac12 e^{td_i}+\frac12e^{-td_i}}
...
\begin{corollary}[(Weak Law of Large Numbers with exponential bound)] 
\def\ceq#1#2#3{\parbox[t]{45ex}{$\displaystyle #1$}\medrel{#2}{$\displaystyle #3$}}
Let $\Pr$ be a probability measure on $\U$.
Then, for every event $\B\subseteq\U$ and every $n,\epsilon>0$
...
\begin{theorem}[(Uniform law of large numbers)]\label{thm_vc}
...
\ceq{\hfill\exp(-\frac{\epsilon^2}{2|\B\cap A|})}{\le}{\frac1{m+2}.}

\ceq{}{\Updownarrow}{} 

\ceq{\hfill2|\B\cap A|\ln(m+2)}{\le}{\epsilon^2}

As $|\B|\le n$, the latter is clearly true.
\end{proof}

\begin{void_thm}[Proof of Corollary \ref{corol_epsilon_approx}]\rm
...
\ceq{\hfill2\sum^h_{i=1}\delta_i}{\le}{2\sum^h_{i=1}\sqrt{(2/n_i)\ln(e\,n_i^k+2)}}

and, as $n_i=2^{-i}n_0$, the sum above is proportional to its largest term,

\ceq{}{\le}{c\sqrt{(k/n_h)\ln(n_h)}}

Hence a sufficient condition for $\epsilon_h<\epsilon$ is 

\ceq{\hfill c\sqrt{(k/n_h)\ln(n_h)}}{\le}{\epsilon}

which is equivalent to

\ceq{\hfill \frac{c^2 k}{\epsilon^2}}{\le}{\frac{n_h}{\ln n_h}}

Let $h$ be the maximal (or, $n_h$ the minimal) that satisfies this inequality. Hence


\ceq{\hfill \frac{c^2 k}{\epsilon^2}}{>}{\frac{n_{h+1}}{\ln n_{h+1}}}

By Proposition~\ref{prop_disequazione}

\ceq{\hfill n_{h+1}}{\le}{ 2\frac{c^2 k}{\epsilon^2}\ln\frac{c^2 k}{\epsilon^2}}

hence 

\ceq{\hfill n_h}{\le}{ 4\frac{c^2 k}{\epsilon^2}\ln\frac{c^2 k}{\epsilon^2}}
