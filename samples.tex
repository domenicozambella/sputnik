...
\begin{void_thm}[Proposition (Chebychev's inequality)]\label{wlln}
...
\ceq{\hfill1-\frac{{\rm Var}(X)}{\epsilon^{\scriptscriptstyle 2}}}{<}{\displaystyle\Pr \Big(\big|X-{\rm E}(X)\big|\le\epsilon\Big)}
...
\ceq{\hfill1-\frac{1}{4n\epsilon^{\scriptscriptstyle 2}}}{<}{\displaystyle{\Pr}^n \Big(c\in\U^n\ :\ \Big|\Pr(\B) - \Fr(\B/c) \Big|\le\epsilon\Big)}
\end{void_thm}
...
\ceq{\hfill \Pr\big(M\ge\epsilon\big)}{\le}{\exp(-\frac{\epsilon^2}{2n})}\hfill where $\displaystyle M=\sum^n_{i=1}X_i$
\end{lemma}
\begin{proof}
