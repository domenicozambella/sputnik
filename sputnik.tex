...
\newtheoremstyle{mio}% name
     ...
     {1.5ex}%     Space after thm head: " " = normal interword space;
           %   \newline = linebreak
     {\llap{\thmnumber{#2}\hskip2mm}\thmname{#1}\thmnote{\kern1ex\bfseries #3}}% Thm head spec (can be left empty, meaning `normal')
...
\begin{document}
\raggedbottom
\renewcommand{\contentsname}{Topics around Vapnik-Chevronenkis theory}
\tableofcontents

\bigskip
This is a subset of the content of the first chapters of \href{http://www.math.ucla.edu/~chernikov/teaching/Combinatorics285N/CombinatoricsNotes.pdf}{Artem Chernicov's Topics in combinatorics} which is also the main source of these notes.

These notes follow closely my expositions in the class (some proofs are different from Chernikov's). However, my notes are less complete and in general less reliable. So, students are encouraged to study from both.
\subfile{sauer}
\subfile{samples}
\subfile{helly}
%\subfile{pearcing}
